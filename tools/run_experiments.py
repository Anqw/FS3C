...
def get_config(seed, shot):
    ...
    temp = os.path.join(
        temp_split, 'faster_rcnn_R_101_FPN_ft_{}_1shot'.format(
            temp_mode)
    )
    config = os.path.join(args.root, config_dir, temp + '.yaml')

    prefix = 'faster_rcnn_R_101_FPN_ft_{}_{}shot{}'.format(
        mode, shot, args.suffix)
    ...
    if args.coco and not args.novel_finetune:
        ...
        configs['MODEL']['BASE'] = configs['MODEL']['BASE'].replace(
            'base1', 'base' + str(args.split))
